...
#define MAX_PROBERESP_LEN 768
    ...
    if (hapd->conf->proberesp_elements) {
      wpa_printf(MSG_DEBUG, "%s:%s:%d:proberesp_elements found",
        __FILE__, __FUNCTION__, __LINE__);
      buflen += wpabuf_len(hapd->conf->proberesp_elements);
		}
		else {
			wpa_printf(MSG_DEBUG, "%s:%s:%d:vendor_elements found and used for probe response",
				__FILE__, __FUNCTION__, __LINE__);
  ...
  if (hapd->conf->proberesp_elements) {
    os_memcpy(pos, wpabuf_head(hapd->conf->proberesp_elements),
      wpabuf_len(hapd->conf->proberesp_elements));
    pos += wpabuf_len(hapd->conf->proberesp_elements);
