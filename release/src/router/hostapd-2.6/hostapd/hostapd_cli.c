...
static int hostapd_cli_cmd_set_vsie(struct wpa_ctrl *ctrl,
  int argc, char *argv[])
{
  char cmd[256];
  int res;
  ...
  printf("hostapd_cli_cmd_set_vsie: argc: %d\n", argc);
  if (argc < 2) {
    printf("Invalid SET_VSIE command\n"
      "usage: type DD<data length><OUI><hexField>\n");
    return -1;
  }

  res = os_snprintf(cmd, sizeof(cmd), "SET_VSIE %s", argv[0]);
  if (res < 0 || (size_t)res >= sizeof(cmd) - 1) {
    printf("Too long SET_VSIE command.\n");
    return -1;
  }
...
static int hostapd_cli_cmd_del_vsie(struct wpa_ctrl *ctrl,
  int argc, char *argv[])
{
  char cmd[256];
  int res;
  ...
  printf("hostapd_cli_cmd_del_vsie: argc: %d\n", argc);
  if (argc < 2) {
    printf("Invalid DEL_VSIE command\n"
      "usage: type DD<data length><OUI><hexField>\n");
    return -1;
  }

  res = os_snprintf(cmd, sizeof(cmd), "DEL_VSIE %s", argv[0]);
  if (res < 0 || (size_t)res >= sizeof(cmd) - 1) {
    printf("Too long DEL_VSIE command.\n");
    return -1;
  }
...
static const struct hostapd_cli_cmd hostapd_cli_commands[] = {
  ...
  { "set_vsie", hostapd_cli_cmd_set_vsie, NULL,
    " <managementFrameId> <VSIE>"
    " managementFrameId: 0:Beacon 1:ProbeResponse 2: AuthenticationResponse 3: AssociationResponse"
    " [set VSIE for managment frame] hostapd_cli -iwlan0 0 dd050009860100" },
  { "del_vsie", hostapd_cli_cmd_del_vsie, NULL,
    " <managementFrameId> <VSIE>"
    " managementFrameId: 0:Beacon 1:ProbeResponse 2: AuthenticationResponse 3: AssociationResponse"
    " [del VSIE for managment frame] hostapd_cli -iwlan0 0 dd050009860100" },
#ifdef CONFIG_MBO
