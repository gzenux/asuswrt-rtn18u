...
xdr___rpc_qcsapi_ap_properties (XDR *xdrs, __rpc_qcsapi_ap_properties *objp)
{
	...
	if (xdrs->x_op == XDR_ENCODE) {
		...
		IXDR_PUT_LONG(buf, objp->ap_dtimperiod);
		IXDR_PUT_LONG(buf, objp->ap_11b_present);
		}
		...
		objp->ap_dtimperiod = IXDR_GET_LONG(buf);
		objp->ap_11b_present = IXDR_GET_LONG(buf);
		}
		 if (!xdr_vector (xdrs, (char *)objp->ap_basic_rates, 256,
			sizeof (uint8_t), (xdrproc_t) xdr_uint8_t))
			 return FALSE;
...
bool_t
xdr_qcsapi_bsa_get_sta_stats_rpcdata (XDR *xdrs, qcsapi_bsa_get_sta_stats_rpcdata *objp)
{
	register int32_t *buf;

	 if (!xdr_int (xdrs, &objp->stat_buflen))
		 return FALSE;
	 if (!xdr_pointer (xdrs, (char **)&objp->stat_buf, sizeof (__rpc_string), (xdrproc_t) xdr___rpc_string))
		 return FALSE;
	 if (!xdr_int (xdrs, &objp->return_code))
		 return FALSE;
	return TRUE;
