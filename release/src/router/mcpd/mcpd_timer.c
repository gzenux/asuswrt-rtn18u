...
// Now we're doing the NON CMS build, so we have to work this pretty hard
#include <stdlib.h>
#include <stdio.h>
#include <time.h>
#include <unistd.h>
 ...
 *   CMS stubs            ***************************
 *                                                  *
 *  The stubs go above the mcpd_timer code because  *
 *  they should only use the external functions.    *
 *                                                  *
 *  "Because we all want  ***************************
 *  to write good code."  *
 **************************/
...
CmsRet cmsTmr_set(void *tmrHandle, McpdEventHandler func, void *ctxData, unsigned int ms, const char *name)
{
  mcpd_timer_new(ms, func, ctxData);
  return CMSRET_SUCCESS;
}

int cmsTmr_isEventPresent(const void *handle, CmsEventHandler func, void *ctxData)
{
  if (mcpd_get_timer(func, ctxData) == NULL) {
    return 0;
  }
  return 1;
}

void cmsTmr_cancel(void *tmrHandle, McpdEventHandler func, void *ctxData)
{
  mcpd_timer_cancel(func, ctxData);
}
...
CmsRet cmsTmr_getTimeToNextEvent(const void *tmrHandle, unsigned int *ms)
{
  struct timeval time_out;
  struct timeval *ptime;
  ptime = mcpd_timer_timeToNextEvent_msec(&time_out);
  // translate ptime to UINT32ms
  if (ptime) {
    *ms = ptime->tv_sec*1000 + ptime->tv_usec/1000;
  }
  else {
    *ms = UINT32_MAX;
  }
...
CmsRet cmsTmr_getTimeRemaining(const void *handle, McpdEventHandler func, void *ctxData, unsigned int *ms)
{
  *ms = mcpd_timer_getTimeRemaining_msec(func, ctxData);
  return CMSRET_SUCCESS;
}
