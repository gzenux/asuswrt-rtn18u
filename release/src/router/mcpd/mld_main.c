...
t_MCPD_RET_CODE mcpd_mld_process_input(t_MCPD_PKT_INFO *pkt_info)
{
    ...
    switch (mldv1->icmp6_hdr.icmp6_type) 
    {
        case ICMPV6_MLD_V1V2_QUERY:
            ...
            else if(pkt_info->data_len >= sizeof(*mld2_qry))
            {
                                             ...
                                             t_MCPD_PKT_INFO *pkt_info)
{
    ...
    if(report->icmp6_hdr.icmp6_type == ICMPV6_MLD_V1_REPORT)
    {
        ...
        if(CMSRET_SUCCESS != cmsTmr_set(tmrHandle, 
                                      ...
                                      gp,
                                      mcpd_router.mld_config.query_interval * mcpd_router.mld_config.robust_val * MSECS_IN_SEC +
...
void mpcd_mld_last_member_query_tmr(void *handle)
{
   ...
   else
   {
      ...
      if(CMSRET_SUCCESS != cmsTmr_set(tmrHandle,
                                      ...
                                      gp,
                                      mcpd_router.mld_config.lmqi * (MSECS_IN_SEC/10),
                                      "mpcd_mld_lmq_tmr"))
      {
...
void mpcd_mld_start_last_member_query_tmr(t_MCPD_INTERFACE_OBJ *ifp,
                                          t_MCPD_GROUP_OBJ *gp)
{     
