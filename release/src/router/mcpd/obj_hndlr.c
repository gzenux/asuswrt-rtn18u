                      ...
                      t_MCPD_REP_OBJ   *rep)
{
    ...
    if (proto == MCPD_PROTO_IGMP)
    {
       if(cmsTmr_isEventPresent(tmrHandle, mcpd_igmp_timer_reporter, rep))
       {
           cmsTmr_cancel(tmrHandle, mcpd_igmp_timer_reporter, rep);
       }
    }
#ifdef SUPPORT_MLD
    else if (proto == MCPD_PROTO_MLD)
    {
       if(cmsTmr_isEventPresent(tmrHandle, mcpd_mld_timer_reporter, rep))
       {
           cmsTmr_cancel(tmrHandle, mcpd_mld_timer_reporter, rep);
       }
    }
#endif
          ...
          int filter)
{
    ...
    if(proto == MCPD_PROTO_IGMP)
    {
        if(cmsTmr_isEventPresent(tmrHandle, mcpd_igmp_timer_source, src))
        {
            cmsTmr_cancel(tmrHandle, mcpd_igmp_timer_source, src);
        }
    }
#ifdef SUPPORT_MLD
    else
    {
        if(cmsTmr_isEventPresent(tmrHandle, mcpd_mld_timer_source, src))
        {
            cmsTmr_cancel(tmrHandle, mcpd_mld_timer_source, src);
        }
    }
#endif /* SUPPORT_MLD */
            ...
            t_MCPD_GROUP_OBJ* gp)
{
    ...
    if(proto == MCPD_PROTO_IGMP)
    {
        ...
        if(cmsTmr_isEventPresent(tmrHandle, mpcd_igmpv2_last_member_query_tmr, gp))
        {
            cmsTmr_cancel(tmrHandle, mpcd_igmpv2_last_member_query_tmr, gp);
        }

        if(cmsTmr_isEventPresent(tmrHandle, mcpd_igmp_timer_group, gp))
        {
            cmsTmr_cancel(tmrHandle, mcpd_igmp_timer_group, gp);
        }

        if(cmsTmr_isEventPresent(tmrHandle, mcpd_igmp_v2_bckcomp_tmr, gp))
        {
            cmsTmr_cancel(tmrHandle, mcpd_igmp_v2_bckcomp_tmr, gp);
        }

        if(cmsTmr_isEventPresent(tmrHandle, mcpd_igmp_v1_bckcomp_tmr, gp))
        {
            cmsTmr_cancel(tmrHandle, mcpd_igmp_v1_bckcomp_tmr, gp);
        }
    }
#ifdef SUPPORT_MLD
    else
    {
        ...
        if(cmsTmr_isEventPresent(tmrHandle, mpcd_mld_last_member_query_tmr, gp))
        {
            cmsTmr_cancel(tmrHandle, mpcd_mld_last_member_query_tmr, gp);
        }

        if(cmsTmr_isEventPresent(tmrHandle, mcpd_mld_timer_group, gp))
        {
            cmsTmr_cancel(tmrHandle, mcpd_mld_timer_group, gp);
        }

        if(cmsTmr_isEventPresent(tmrHandle, mcpd_mld_v1_bckcomp_tmr, gp))
        {
            cmsTmr_cancel(tmrHandle, mcpd_mld_v1_bckcomp_tmr, gp);
        }
    }
#endif /* SUPPORT_MLD */
...
int mcpd_port_group_count(int rep_ifi, t_MCPD_INTERFACE_OBJ *ifp)
{
   ...
   MCPD_TRACE(MCPD_TRC_INFO, "Counting groups for interface %d", rep_ifi);

   /* count number of groups already on that port */
   t_MCPD_GROUP_OBJ* mcpdGroup = ifp->igmp_proxy.groups;
...
#ifdef SUPPORT_MLD
      if ((!mcpdGroup) && (!mldLoop))
      {
         // Go through all of the MLD groups and count them, too.
         mcpdGroup = ifp->mld_proxy.groups;
         mldLoop = 1;
      }
#endif
   }

   MCPD_TRACE(MCPD_TRC_INFO, "Found %d groups", groupCount);
...
int mcpd_port_max_group_count(int rep_ifi)
{
   ...
   for ( ; i < MAX_PORTS; i++)
   {
      ...
      {              
         return mcpd_router.max_group_port_list[i].maxPorts;
      }
   }
   return -1;
}
...
{
    int num_grps = mcpd_interface_group_count(proto, ifp);
                               ...
                               int rep_ifi)
{
    ...
    if (MCPD_TRUE != mcpd_is_there_space_for_group(MCPD_PROTO_IGMP, rep_ifi, ifp) )
    {
        return NULL;
    }
...
UINT32 mcpd_get_largest_timeout_secs (t_MCPD_GROUP_OBJ *gp, CmsEventHandler func)
{
   ...
   while (rep) {
      UINT32 timeout;
      cmsTmr_getTimeRemaining(tmrHandle, func, rep, &timeout);
...
void mcpd_dump_obj_tree(void)
{
    ...
    for(ifp = mcpd_router.interfaces; ifp; ifp = ifp->next)
    {
        ...
        for(gp = ifp->igmp_proxy.groups; gp; gp = gp->next)
        {
            ...
            {
                cmsTmr_getTimeRemaining(tmrHandle, mcpd_igmp_timer_source, src, &timeOut);
            ...
            {
                cmsTmr_getTimeRemaining(tmrHandle, mcpd_igmp_timer_reporter, rep, &timeOut);
        ...
        for(gp = ifp->mld_proxy.groups; gp; gp = gp->next)
        {
            ...
            for(src = gp->in_sources; src; src = (t_MCPD_SRC_OBJ *)src->next)
            {
                ...
                cmsTmr_getTimeRemaining(tmrHandle, mcpd_mld_timer_source, src, &timeOut);
                timeOut /= MSECS_IN_SEC;
            ...
            for (rep = (t_MCPD_REP_OBJ *) gp->members; rep;
                                     rep = (t_MCPD_REP_OBJ *)rep->next)
            {
                cmsTmr_getTimeRemaining(tmrHandle, mcpd_mld_timer_reporter, rep, &timeOut);
                printf("         Reporter ");
