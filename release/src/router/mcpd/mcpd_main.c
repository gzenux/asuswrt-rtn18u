...
void mcpd_input_run(void)
{
    ...
    time_out.tv_sec = nextEventMs / MSECS_IN_SEC;
    time_out.tv_usec = (nextEventMs % MSECS_IN_SEC) * USECS_IN_MSEC;
    n = select(maxfd+1, &rfds, NULL, NULL, &time_out);
...
void mcpd_process_query_timer(void *handle)
{
    ...
    if(!cmsTmr_isEventPresent(tmrHandle, mcpd_process_query_timer, NULL))
    {
        if(CMSRET_SUCCESS != cmsTmr_set(tmrHandle,
                                mcpd_process_query_timer,
                                NULL,
                                (timer_val * MSECS_IN_SEC),
                                "mcpd_process_query_timer"))
        {
...
void mcpd_update_interface_info(void)
{
    ...
    for(if_intf = mcpd_router.interfaces; if_intf; )
    {
           ...
           (if_intf->audit_done == MCPD_FALSE))
        {
            ...
            /* clean up iptables */
            /* IPV4 filter and NAT */
            sprintf(cmd, "iptables -t filter -D FORWARD -i %s "
                         "-d 224.0.0.0/4 -j ACCEPT 2>/dev/null; ",
                         if_intf->if_name );
            system(cmd);
            sprintf(cmd, "iptables -t nat -D PREROUTING -i %s "
                         "-d 224.0.0.0/4 -j ACCEPT 2>/dev/null; ",
                         if_intf->if_name );
            system(cmd);
#ifdef SUPPORT_MLD
            /* IPV6 filter */
            sprintf(cmd, "ip6tables -t filter -D FORWARD -i %s "
                         "-d FF00::0/8 -j ACCEPT 2>/dev/null; ",
                         if_intf->if_name );
            system(cmd);
#endif
            /* igmp packets */
            sprintf(cmd, "iptables -t filter -D INPUT -i %s "
        ...
        if (if_intf->if_dir == MCPD_UPSTREAM)
        {
            ...
            if ( igmpSourceEnabled == MCPD_TRUE )
            {
                ...
                if (if_intf->if_type & MCPD_IF_TYPE_ROUTED)
                {
                    sprintf(cmd, "iptables -t filter -D FORWARD -i %s "
                                 "-d 224.0.0.0/4 -j ACCEPT 2>/dev/null; "
                                 "iptables -t filter -I FORWARD 1 -i %s "
                                 "-d 224.0.0.0/4 -j ACCEPT 2>/dev/null",
            ...
            if ( mldSourceEnabled == MCPD_TRUE )
            {
                ...
                if (if_intf->if_type & MCPD_IF_TYPE_ROUTED)
                {
                    sprintf(cmd, "ip6tables -t filter -D FORWARD -i %s "
                                 "-d FF00::0/8 -j ACCEPT 2>/dev/null; "
                                 "ip6tables -t filter -I FORWARD 1 -i %s "
                                 "-d FF00::0/8 -j ACCEPT 2>/dev/null",
                                 if_intf->if_name, if_intf->if_name );
                    system(cmd);
                }
            ...
            if ( igmpProxyEnabled == MCPD_TRUE )
            {
                if (if_intf->if_addr.s_addr != 0)
                {
                    ...
                    sprintf(cmd, "iptables -t filter -D INPUT -i %s "
                                 "-p 2 -j ACCEPT 2>/dev/null; "
                                 "iptables -t filter -I INPUT 1 -i %s "
                                 "-p 2 -j ACCEPT 2>/dev/null",
                                 if_intf->if_name, if_intf->if_name );
                    system(cmd);
                }
            ...
            if ( mldProxyEnabled == MCPD_TRUE )
            {
                if (!IN6_IS_ADDR_UNSPECIFIED(&if_intf->if_addr6))
                {
                    ...
                    /* accept MLD packets */
                    sprintf(cmd, "ip6tables -t filter -D INPUT -i %s -p icmpv6 "
                                 "--icmpv6-type 130 -j ACCEPT 2>/dev/null; "
                                 "ip6tables -t filter -I INPUT 1 -i %s -p icmpv6 "
                                 "--icmpv6-type 130 -j ACCEPT 2>/dev/null",
                                 if_intf->if_name, if_intf->if_name );
                    system(cmd);
                }
...
void mcpd_update_multicast_info()
{
    ...
    bcm_mcast_api_send_group_timeout (mcpd_router.sock_nl, IPPROTO_IGMP, 
                                      mcpd_router.igmp_config.query_interval * mcpd_router.igmp_config.robust_val + 
...
#ifdef BRCM_CMS_BUILD
    if ((cmsTmr_init(&tmrHandle)) != CMSRET_SUCCESS)
    {
        MCPD_TRACE(MCPD_TRC_ERR, "cmsTmr_init failed");
    }
#endif
