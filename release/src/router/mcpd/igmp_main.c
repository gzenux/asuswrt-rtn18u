...
t_MCPD_RET_CODE mcpd_igmp_process_input(t_MCPD_PKT_INFO *pkt_info)
{
   ...
   if (ifp == NULL)
   {
      MCPD_TRACE(MCPD_TRC_ERR, "Unexpected packet from interface %d\n", pkt_info->parent_ifi);
      return MCPD_RET_GENERR;
   }
   ...
   if ( 0x00000000 == pkt_info->ipv4rep.s_addr )
   {
      /* ignore this report */
      return MCPD_RET_GENERR;
   }
   ...
   switch (report12->type)
   {
      case IGMP_MEMBERSHIP_QUERY:
         ...
         else if (pkt_info->data_len >= sizeof(*query3))
         {
            /* version 3 query */
      ...
      case IGMP_V3_MEMBERSHIP_REPORT:
         if(pkt_info->data_len < sizeof(*report3))
         {
                                               ...
                                               t_MCPD_PKT_INFO *pkt_info)
{
   ...
   if(report->type == IGMP_V2_MEMBERSHIP_REPORT)
   {
      ...
      if(CMSRET_SUCCESS != cmsTmr_set(tmrHandle,
                                      ...
                                      gp,
                                      mcpd_router.igmp_config.query_interval * mcpd_router.igmp_config.robust_val * MSECS_IN_SEC +
...
void mpcd_igmpv2_last_member_query_tmr(void *handle)
{
   ...
   else
   {
      ...
      if(CMSRET_SUCCESS != cmsTmr_set(tmrHandle,
                                      ...
                                      gp,
                                      mcpd_router.igmp_config.lmqi * (MSECS_IN_SEC/10),
                                      "mpcd_igmpv2_lmq_tmr"))
      {
...
void mpcd_igmpv2_start_last_member_query_tmr(t_MCPD_INTERFACE_OBJ *ifp,
                                             t_MCPD_GROUP_OBJ *gp)
{
