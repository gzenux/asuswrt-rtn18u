...
char *get_usb_xhci_port(int port)
{
    ...
    if (!ports_string || strlen(ports_string) == 0)
    {
        xhci_string[0] = '\0'; // Indicate no data
        return NULL;
    }

    // Initialize with xxxxxxxx
    strcpy(xhci_string, "xxxxxxxx");

    foreach (word, ports_string, next)
    {
        if (i == port)
        {
            strncpy(xhci_string, word, sizeof(xhci_string) - 1);
            xhci_string[sizeof(xhci_string) - 1] = '\0';
            return xhci_string;
        }
        i++;
    }

    // If no match is found, return NULL or keep "xxxxxxxx"
    return xhci_string[0] ? xhci_string : NULL;
}
...
char *get_usb_ehci_port(int port)
{
    ...
    foreach (word, ports_string, next)
    {
        if (i == port)
        {
            strncpy(ehci_string, word, sizeof(ehci_string) - 1);
            ehci_string[sizeof(ehci_string) - 1] = '\0';
            return ehci_string;
        }
        i++;
    }
    ...
    // If no match is found, return NULL or keep "xxxxxxxx"
    return ehci_string[0] ? ehci_string : NULL;
}
...
char *get_usb_ohci_port(int port)
{
    ...
    foreach (word, ports_string, next)
    {
        if (i == port)
        {
            strncpy(ohci_ports, word, sizeof(ohci_ports) - 1);
            ohci_ports[sizeof(ohci_ports) - 1] = '\0';
            return ohci_ports;
        }
        i++;
    }
    ...
    // If no match is found, return NULL or keep "xxxxxxxx"
    return ohci_ports[0] ? ohci_ports : NULL;
}
// #endif
...
int get_disk_partitionnumber(const char *string, u32 *partition_number, u32 *mounted_number)
{
    ...
    *partition_number = 0; // initial value.
    ...
    int written = snprintf(target_path, 128, "%s/%s", SYS_BLOCK, disk_name);
    if (written >= 128)
    {
        if (mount_info != NULL)
            free(mount_info);
    ...
    while ((file = readdir(dp)) != NULL)
    {
        ...
        if (!strncmp(file->d_name, disk_name, len))
        {
            ...
            memset(target, 0, 8);
            snprintf(target, 8, "%s ", file->d_name);
            if (strstr(mount_info, target) != NULL)
                ++(*mounted_number);
        }
    }
...
void FAST_FUNC bb_verror_msg(const char *s, va_list p, const char *strerr)
{
    ...
    if (!msg1)
    {
        if (used < sizeof(msg) - 2)
        {                       /* ensure there is room for '\n' and '\0' */
            msg[used++] = '\n'; /* overwrites NUL */
            msg[used] = '\0';   /* ensure string is null-terminated */
            applet_len = 0;
        }
    }
    else
...
void FAST_FUNC xfunc_die(void)
{
    if (die_sleep)
    {
        if ((ENABLE_FEATURE_PREFER_APPLETS || ENABLE_HUSH) && die_sleep < 0)
        {
...
#define off ((uint64_t)0)
    ...
    for (page = 0x1000; page <= LARGEST_PAGESIZE; page <<= 1)
    {
        ...
        if (memcmp(buf, "SWAPSPACE2", 10) == 0 || memcmp(buf, "S1SUSPEND", 9) == 0 || memcmp(buf, "S2SUSPEND", 9) == 0 || memcmp(buf, "ULSUSPEND", 9) == 0)
        {
...
#define fat_partition_off ((uint64_t)0)
    ...
    if ((vs->boot_jump[0] != 0xeb || vs->boot_jump[2] != 0x90) && vs->boot_jump[0] != 0xe9)
    {
        return -1;
    }
    ...
    /* cluster size check */
    if (vs->sectors_per_cluster == 0 || (vs->sectors_per_cluster & (vs->sectors_per_cluster - 1)))
    {
        return -1;
    }
...
valid:
    ...
    if (sector_size_bytes != 0x200 && sector_size_bytes != 0x400 && sector_size_bytes != 0x800 && sector_size_bytes != 0x1000)
    {
        return -1;
    }
...
hfsplus:
    ...
    bnode = (struct hfsplus_bheader_record *)&buf[sizeof(struct hfsplus_bnode_descriptor)];
    ...
    key = (struct hfsplus_catalog_key *)&buf[sizeof(struct hfsplus_bnode_descriptor)];
...
int find_partition_label(const char *dev_name, char *label)
{
    ...
    snprintf(nvram_label, 32, "usb_path_%s_label", dev_name);
    ...
    snprintf(dev_path, 128, "/dev/%s", dev_name);
...
int read_mount_data(const char *device_name, char *mount_point, int mount_len, char *type, int type_len, char *right, int right_len)
{
    char *mount_info = read_whole_file(MOUNT_FILE);
    char *start, line[PATH_MAX];
    char target[8];

    if (mount_point == NULL || mount_len <= 0 || type == NULL || type_len <= 0 || right == NULL || right_len <= 0)
    {
        usb_dbg("Bad input!!\n");
        return 0;
    }
    ...
    memset(target, 0, 8);
    snprintf(target, 8, "%s ", device_name);
    ...
    if (!strcmp(type, "ufsd"))
    {
        ...
        snprintf(full_dev, 16, "/dev/%s", device_name);
...
int get_partition_size(const char *partition_name, u64 *size_in_kilobytes)
{
    ...
    *size_in_kilobytes = 0; // initial value.
    ...
    int written = snprintf(target_file, 128, "%s/%s/%s/size", disk_name, partition_name);
    if (written >= 128)
    {
        return 0;
    }
...
partition_info_t *create_partition(const char *device_name, partition_info_t **new_part_info)
{
    ...
    *new_part_info = NULL; // initial value.
    ...
    if (len >= 0)
    {
        //- Make sure follow_part_info->device is null terminated.
    ...
    if (find_partition_label(device_name, label))
    {
        ...
        if (len >= 0)
        {
            //- Make sure follow_part_info->label is null terminated.
    ...
    if (read_mount_data(device_name, buf1, PATH_MAX, buf2, 64, buf3, PATH_MAX))
    {
        ...
        if (len >= 0)
        {
            //- Make sure follow_part_info->mount_point is null terminated.
        ...
        if (len >= 0)
        {
            //- Make sure follow_part_info->file_system is null terminated.
        ...
        if (len >= 0)
        {
            //- Make sure follow_part_info->permission is null terminated.
...
int get_disk_major_minor(const char *disk_name, u32 *major, u32 *minor)
{
    ...
    *minor = 0; // initial value.
    ...
    memset(target_file, 0, 128);
    int written = snprintf(target_file, 128, "%s/%s/dev", SYS_BLOCK, disk_name);
    if (written >= 128)
    {
        return 0;
    }
...
int get_disk_size(const char *disk_name, u64 *size_in_kilobytes)
{
    ...
    *size_in_kilobytes = 0; // initial value.
    ...
    memset(target_file, 0, 128);
    int written = snprintf(target_file, 128, "%s/%s/size", SYS_BLOCK, disk_name);
    if (written >= 128)
    {
        return 0;
    }
...
char *get_usb_node_by_device(const char *device_name, char *buf, const int buf_size)
{
    ...
    if (device_type == DEVICE_TYPE_DISK)
    {
        ...
        int written = snprintf(device_path, 128, "%s/%s/device", SYS_BLOCK, disk_name);
        if (written >= 128)
        {
            return NULL;
        }
    ...
    {
        snprintf(device_path, PATH_MAX, "%s/%s/device", SYS_USB, device_name);
        if (realpath(device_path, usb_path) == NULL)
        {
            usb_dbg("(%s): Fail to get link: %s.\n", device_name, device_path);
            return NULL;
    ...
    {
        snprintf(device_path, 128, "%s/%s/device", SYS_SG, device_name);
        if (realpath(device_path, usb_path) == NULL)
        {
            usb_dbg("(%s): Fail to get link: %s.\n", device_name, device_path);
            return NULL;
    ...
    else if (device_type == DEVICE_TYPE_CD)
    {

        int written = snprintf(device_path, 128, "%s/%s/device", SYS_BLOCK, device_name);
        if (written >= 128)
        {
            return NULL;
        }
    ...
    else if (device_type == DEVICE_TYPE_MODEM)
    {
        snprintf(device_path, 128, "%s/%s/device", SYS_TTY, device_name);
        if (realpath(device_path, usb_path) == NULL)
        {
    ...
    {
        snprintf(device_path, 128, "%s/%s/device", SYS_USB, device_name);
        if (realpath(device_path, usb_path) == NULL)
        {
            if (realpath(device_path, usb_path) == NULL)
            {
...
char *get_disk_vendor(const char *disk_name, char *buf, const int buf_size)
{
    ...
    memset(target_file, 0, 128);
    int written = snprintf(target_file, 128, "%s/%s/device/vendor", SYS_BLOCK, disk_name);
    if (written >= 128)
    {
        return NULL;
    }
...
char *get_disk_model(const char *disk_name, char *buf, const int buf_size)
{
    ...
    memset(target_file, 0, 128);
    int written = snprintf(target_file, 128, "%s/%s/device/model", SYS_BLOCK, disk_name);
    if (written >= 128)
    {
        return NULL;
    }
...
extern int add_account(const char *const account, const char *const password)
{
    ...
    {
        ptr = nvram_value + len;
        snprintf(ptr, PATH_MAX - len, "<%s>%s", ascii_user, ascii_passwd);
        nvram_set("acc_list", nvram_value);

        snprintf(nvram_value, PATH_MAX, "%d", acc_num + 1);
        nvram_set("acc_num", nvram_value);
    }
    else
    {
        snprintf(nvram_value, PATH_MAX, "%s>%s", ascii_user, ascii_passwd);
        nvram_set("acc_list", nvram_value);
