...
/* home realm */
#define HOME_REALM "t-mobile.com"

#define URL	"https://t-mobile.com"

#define OSU_SERVER_URI "https://osu-server.t-mobile.com/"

#define WIFI_VENUE "T-Mobile"

#define ENGLISH_FRIENDLY_NAME "T-Mobile"
...
#define ENG_OPNAME_SP_RED		"T-Mobile Red Test Only"
#define ENG_OPNAME_SP_BLUE		"T-Mobile Blue Test Only"
#define ENG_OPNAME_SP_GREEN		"T-Mobile Green Test Only"
#define ENG_OPNAME_SP_ORANGE		"T-Mobile Orange Test Only"
#define ENG_OPNAME_WBA			"T-Mobile WBA Test Only"
...
#define OSU_SERVICE_DESC_ID1		"T-Mobile Passpoint service"

#define OSU_NAI_TEST_WIFI		"test-anonymous@t-mobile.com"
#define OSU_NAI_ANON_HS			"anonymous@t-mobile.com"
...
int Reset_hspot_flag(char *prefix, bool bInit)
{
	...
	if (!bInit)
	{
		...
		if (ret) {
			dbg("nvram_unset %s failure\n", varname);
			err = -1;
		}
	}
	else
	{
		ret = nvram_set(strcat_r(prefix, "hs2en", varname), "1");
		if (ret) {
			dbg("nvram_set %s=%s failure\n", varname, varvalue);
			err = -1;
		}
		ret = nvram_set(strcat_r(prefix, "u11en", varname), "1");
		if (ret) {
			dbg("nvram_set %s=%s failure\n", varname, varvalue);
			err = -1;
		}
...
int Reset_VenueList(hspotApT *hspotap, bool bInit, unsigned int flag)
{
	...
	else
	{
		...
		if (flag & 0x0001) {
			...
			ret = nvram_set(strcat_r(hspotap->prefix, "venuetype", varname), "0");
			if (ret) {
				dbg("nvram_set %s=%s failure\n", varname, "0");
				err = -1;
			}
		...
		if (flag & 0x0002) {
			...
			ret = nvram_set(strcat_r(hspotap->prefix, "venuegrp", varname), "0");
			if (ret) {
				dbg("nvram_set %s=%s failure\n", varname, "0");
				err = -1;
			}
		...
		if (flag & 0x0004) {
			...
			hspotap->venuelist.numVenueName = 1;
...
int Reset_NatList(hspotApT *hspotap, bool bInit)
{
	...
	else
	{
		...
		ret = nvram_set(strcat_r(hspotap->prefix, "netauthlist", varname),
			"accepttc=+httpred=https://t-mobile.com");
		if (ret) {
			dbg("nvram_set %s=%s failure\n", varname,
				"accepttc=+httpred=https://t-mobile.com");
			err = -1;
...
int Reset_Osuplist(hspotApT *hspotap, bool bInit, unsigned int flag)
{
	...
	else
	{
		...
		/* OSU_SSID */
		if (flag & 0x0001) {
			strncpy_n((char*)hspotap->osuplist.osuSsid, "CellSpot_AutoConnect",
				BCM_DECODE_HSPOT_ANQP_MAX_OSU_SSID_LENGTH + 1);
			hspotap->osuplist.osuSsidLength = strlen("CellSpot_AutoConnect");
			ret = nvram_set(strcat_r(hspotap->prefix, "osu_ssid", varname),
				"CellSpot_AutoConnect");
			if (ret) {
				dbg("nvram_set %s=%s failure\n", varname, "CellSpot_AutoConnect");
				err = -1;
			}
		}
		/* OSU_Friendly_Name */
		...
		if (flag & 0x0040) {
			...
			ret = nvram_set(strcat_r(hspotap->prefix, "osu_servdesc", varname),
				"T-Mobile Passpoint service!eng");
			if (ret) {
				dbg("nvram_set %s=%s failure\n", varname,
				"T-Mobile Passpoint service!eng");
				err = -1;
...
int Reset_Realmlist(hspotApT *hspotap, bool bInit)
{
	...
	hspotap->realmlist.realmCount = 5;
	...
	hspotap->realmlist.realm[1].realmLen = strlen(CISCO);
	strncpy_n((char*)hspotap->realmlist.realm[1].realm, CISCO,
		BCM_DECODE_ANQP_MAX_REALM_LENGTH + 1);
	...
	hspotap->realmlist.realm[2].realmLen = strlen(WIFI);
	strncpy_n((char*)hspotap->realmlist.realm[2].realm, WIFI,
		BCM_DECODE_ANQP_MAX_REALM_LENGTH + 1);
	hspotap->realmlist.realm[2].eapCount = 1;
	/* EAP 3.1 */
	hspotap->realmlist.realm[2].eap[0].eapMethod = (uint8)REALM_EAP_TTLS;
	hspotap->realmlist.realm[2].eap[0].authCount = 2;
	/* Auth 3.1.1 */
	hspotap->realmlist.realm[2].eap[0].auth[0].id = (uint8)REALM_NON_EAP_INNER_AUTHENTICATION;
	hspotap->realmlist.realm[2].eap[0].auth[0].len = sizeof(auth_MSCHAPV2);
	memcpy(hspotap->realmlist.realm[2].eap[0].auth[0].value,
		auth_MSCHAPV2, sizeof(auth_MSCHAPV2));
	/* Auth 3.1.2 */
	hspotap->realmlist.realm[2].eap[0].auth[1].id = (uint8)REALM_CREDENTIAL;
	hspotap->realmlist.realm[2].eap[0].auth[1].len = sizeof(auth_UNAMPSWD);
	memcpy(hspotap->realmlist.realm[2].eap[0].auth[1].value,
		auth_UNAMPSWD, sizeof(auth_UNAMPSWD));
	/* ---------------------------------------- */

	/* Realm 4 ---------------------------------------- */
	hspotap->realmlist.realm[3].encoding = (uint8)REALM_ENCODING_RFC4282;
	hspotap->realmlist.realm[3].realmLen = strlen(WIFI);
	strncpy_n((char*)hspotap->realmlist.realm[3].realm, WIFI,
		BCM_DECODE_ANQP_MAX_REALM_LENGTH + 1);
	hspotap->realmlist.realm[3].eapCount = 1;
	/* EAP 4.1 */
	hspotap->realmlist.realm[3].eap[0].eapMethod = (uint8)REALM_EAP_TLS;
	hspotap->realmlist.realm[3].eap[0].authCount = 1;
	/* Auth 4.1.1 */
	hspotap->realmlist.realm[3].eap[0].auth[0].id = (uint8)REALM_CREDENTIAL;
	hspotap->realmlist.realm[3].eap[0].auth[0].len = sizeof(auth_CERTIFICATE);
	memcpy(hspotap->realmlist.realm[3].eap[0].auth[0].value,
		auth_CERTIFICATE, sizeof(auth_CERTIFICATE));
	/* ---------------------------------------- */

	/* Realm 5 ---------------------------------------- */
	hspotap->realmlist.realm[4].encoding = (uint8)REALM_ENCODING_RFC4282;
	hspotap->realmlist.realm[4].realmLen = strlen(EXAMPLE4);
	strncpy_n((char*)hspotap->realmlist.realm[4].realm, EXAMPLE4,
		BCM_DECODE_ANQP_MAX_REALM_LENGTH + 1);
	hspotap->realmlist.realm[4].eapCount = 1;
	/* EAP 5.1 */
	hspotap->realmlist.realm[4].eap[0].eapMethod = (uint8)REALM_EAP_TLS;
	hspotap->realmlist.realm[4].eap[0].authCount = 1;
	/* Auth 5.1.1 */
	hspotap->realmlist.realm[4].eap[0].auth[0].id = (uint8)REALM_CREDENTIAL;
	hspotap->realmlist.realm[4].eap[0].auth[0].len = sizeof(auth_CERTIFICATE);
	memcpy(hspotap->realmlist.realm[4].eap[0].auth[0].value,
		auth_CERTIFICATE, sizeof(auth_CERTIFICATE));
	/* ---------------------------------------- */
...
int Reset_Oplist(hspotApT *hspotap, bool bInit)
{
	...
	else
	{
		...
		ret = nvram_set(strcat_r(hspotap->prefix, "oplist", varname), "T-Mobile!eng");
		if (ret) {
			dbg("nvram_set %s=%s failure\n", varname, "T-Mobile!eng");
			err = -1;
		}
	}
...
static hspotApT *getHspotApByBSSID(char *HESSIDstr)
{
	...
	if (!strToEther(HESSIDstr, &da)) {
		dbg("wrong format parameter in command dest\n");
		return hspotaps[0];
	}
	...
	bcm_gas_t *gas, int len, uint8 *data)
{
	...
	/* dbg("HS_TRACE : 0 Timestamp for starting processQueryRequest : %llu \n", now ); */
	...
	if (anqp.anqpQueryListLength > 0) {
		...
		if (bcm_decode_anqp_query_list(&ie, &queryList))
			...
			dbg("failed to decode query list\n");
	...
	if (anqp.hspot.queryListLength > 0) {
		...
		if (bcm_decode_hspot_anqp_query_list(&ie, &queryList))
			...
			dbg("failed to decode passpoint query list\n");
	...
	if (anqp.hspot.naiHomeRealmQueryLength > 0) {
		...
		if (bcm_decode_hspot_anqp_nai_home_realm_query(&ie, &realm))
			...
			dbg("failed to decode passpoint hrq\n");
	...
	dbg("%30s  <-----  query response %d bytes %d\n", "",
	...
	/* dbg("HS_TRACE : 100 Timestamp for starting processQueryRequest : %llu \n", then ); */
	/* dbg("===================================================== \n"); */
	/* dbg("HS_TRACE : Final: Timestamp for processQueryRequest : %llu \n", then-now ); */
	/* dbg("===================================================== \n"); */
...
static void gasEventCallback(void *context, bcm_gas_t *gas, bcm_gas_event_t *event)
{
	...
	if (hspotap == NULL) {
		dbg("can't find matched hspotap\n");
		return;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command interworking\n");
		return -1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command internet\n");
		return -1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command accs_net_type\n");
		return -1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		...
		dbg("HESSID is not present\n");
	}
	else {
		hspotap->iw_isHESSIDPresent = TRUE;
		if (!strToEther(argv[0], &hspotap->iw_HESSID)) {
			dbg("wrong format parameter in command hessid\n");
			return -1;
		}
		...
		dbg("HESSID 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n",
			hspotap->iw_HESSID.octet[0], hspotap->iw_HESSID.octet[1],
			hspotap->iw_HESSID.octet[2], hspotap->iw_HESSID.octet[3],
			hspotap->iw_HESSID.octet[4], hspotap->iw_HESSID.octet[5]);
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command venue_type\n");
		return -1;
	}

	hspotap->venuelist.type = atoi(argv[0]);
	dbg("venuelist.type %d\n", hspotap->venuelist.type);
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command venue_grp\n");
		return -1;
	}

	hspotap->venuelist.group = atoi(argv[0]);
	dbg("venuelist.group %d\n", hspotap->venuelist.group);
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command venue_name\n");
		return -1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command anonymous_nai\n");
		strncpy_n(hspotap->anonai.nai,
		"anonymous.com", BCM_DECODE_HSPOT_ANQP_MAX_NAI_SIZE + 1);
	}
	else {
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command osu_provider_list\n");
		osup_id = 1;
	}
	else {
		osup_id = atoi(argv[0]);
		dbg("osu_provider_list id = %d\n", osup_id);
	}
	...
	case 6:
	{
		...
		hspotap->osuplist.osuProvider[1].name.numName = 1;
		...
		ret = nvram_set(strcat_r(hspotap->prefix, "osu_frndname", varname),
			"T-Mobile Green Test Only!eng;"
			"T-Mobile Orange Test Only!eng");
		if (ret) {
			dbg("nvram_set %s=%s failure\n", varname,
			"T-Mobile Green Test Only!eng;"
			"T-Mobile Orange Test Only!eng");
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command osu_icon_tag\n");
		hspotap->osuicon_id = 1;
	}
	else {
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command osu_ssid\n");
		strncpy_n((char*)hspotap->osuplist.osuSsid,
			"CellSpot_AutoConnect", BCM_DECODE_HSPOT_ANQP_MAX_OSU_SSID_LENGTH + 1);
		hspotap->osuplist.osuSsidLength = strlen("CellSpot_AutoConnect");
	}
	else {
		strncpy_n((char*)hspotap->osuplist.osuSsid,
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command osu_server_uri\n");
		strncpy_n(varvalue, "https://osu-server.t-mobile.com",
			BUFFER_SIZE2);
	}
	else {
		while (argv[i])
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command osu_method\n");
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command domain_list\n");
		return -1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command roaming_cons\n");
		return -1;
	}

	if (!strncasecmp(argv[0], "disabled", 8)) {
		dbg("Roaming consortium OI is not present\n");
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command mih\n");
		return -1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command dgaf_disable\n");
		return -1;
	}

	isDgafDisabled = Get_hspot_flag(hspotap->prefix, HSFLG_DGAF_DS);
	inflag = (atoi(argv[0]) != 0);
	dbg("isDgafDisabled %d\n", inflag);
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command icmpv4_echo\n");
		return -1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command plmn_mcc\n");
		return -1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command plmn_mnc\n");
		return -1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	int proxy_arp = 0;
	if (argv[0] == NULL) {
		dbg("missing parameter in proxy_arp\n");
		return -1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	bool bcst_uncst;
	if (argv[0] == NULL) {
		dbg("missing parameter in bcst_uncst\n");
		return -1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in gas_cb_delay\n");
		return -1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in 4_frame_gas\n");
		return -1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command sess_info_url\n");
		return -1;
	}
	...
	if (hspotap->url_len == 0) {
		dbg("sess_info_url: length is zero\n");
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command dest\n");
		return -1;
	}
	...
	dbg("dest 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n", da.octet[0],
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command hs2\n");
		return -1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command u11\n");
		return -1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command p2p_cross_connect\n");
		return -1;
	}

	p2p_cross_enabled = Get_hspot_flag(hspotap->prefix, HSFLG_P2P_CRS);
	enabled = (atoi(argv[0]) != 0);
	dbg("p2p_cross_connect enabled %d\n", enabled);
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command ip_add_type_avail\n");
		return -1;
	}

	ipa_id = atoi(argv[0]);
	dbg("ip_add_type_avail id %d\n", ipa_id);
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command nai_realm_list\n");
		return -1;
	}
	switch (atoi(argv[0]))
	{
	...
	hspotap->realmlist.realm[0].eap[0].eapMethod = (uint8)REALM_EAP_SIM;
	/* Auth 1.1.1 */
	hspotap->realmlist.realm[0].eap[0].auth[0].id = (uint8)REALM_CREDENTIAL;
	hspotap->realmlist.realm[0].eap[0].auth[0].len = sizeof(auth_SIM);
	memcpy(hspotap->realmlist.realm[0].eap[0].auth[0].value,
	...
	/* Realm 4 ---------------------------------------- */
	hspotap->realmlist.realm[3].encoding = (uint8)REALM_ENCODING_RFC4282;
	hspotap->realmlist.realm[3].realmLen = strlen(EXAMPLE4);
	strncpy_n((char*)hspotap->realmlist.realm[3].realm, EXAMPLE4,
		BCM_DECODE_ANQP_MAX_REALM_LENGTH + 1);
	hspotap->realmlist.realm[3].eapCount = 1;
	/* EAP 4.1 */
	hspotap->realmlist.realm[3].eap[0].eapMethod = (uint8)REALM_EAP_TLS;
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command oper_name\n");
		return -1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command wan_metrics\n");
		return -1;
	}

	wanm_id = atoi(argv[0]);
	dbg("wan_metrics id %d\n", wanm_id);
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command conn_cap\n");
		return -1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command net_auth_type\n");
		return -1;
	}

	nat_id = atoi(argv[0]);
	dbg("net_auth_type id %d\n", nat_id);
	...
	if (nat_id == 1)
	{
		...
		strncpy_n(varvalue, "accepttc=+httpred=https://t-mobile.com",
			NVRAM_MAX_VALUE_LEN);
	}
	else if (nat_id == 2)
	{
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command qos_map_set\n");
		return err;
	}

	hspotap->qos_id = atoi(argv[0]);
	dbg("qos_map_set id = %d\n", hspotap->qos_id);

	if (argv[1] != NULL) {
		if (!strToEther(argv[1], &addr)) {
			dbg("<addr> format is 00:11:22:33:44:55\n");
			return -1;
		}
		sta_mac = 1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command bss_load\n");
		return -1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command pause\n");
		return -1;
	}

	hspotap->isGasPauseForServerResponse = (atoi(argv[0]) != 0);
	bcm_gas_set_if_gas_pause(hspotap->isGasPauseForServerResponse, hspotap->ifr);
	dbg("isGasPauseForServerResponse %d\n", hspotap->isGasPauseForServerResponse);
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command dis_anqp_response\n");
		return -1;
	}

	disable_ANQP_response = (atoi(argv[0]) != 0);
	dbg("disable_ANQP_response %d\n", disable_ANQP_response);
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command sim\n");
		return -1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command response_size\n");
		return -1;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command pause_cb_delay\n");
		return -1;
	}

	pause_cb_delay = atoi(argv[0]);
	dbg("pause_cb_delay %d\n", pause_cb_delay);
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if ((argv[0] == NULL) || (argv[1] == NULL) || (argv[2] == NULL)) {
		dbg("Invalid Number of Parameters\n");
		return err;
	}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	if (argv[0] == NULL) {
		dbg("missing parameter in command btredi\n");
		return -1;
	}
	...
	{
		dbg("wl_wnm_bsstrans_req failed\n");
		return err;
	}
	return err;
}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	dbg("\n");
	dbg("==============================================================================\n");
	dbg("\t\t\tHspotAP Application - CLI Commands \n");
	dbg("==============================================================================\n");
	dbg(" Command 01\t: interface <interface_name> \n"
	...
	dbg("------------------------------------------------------------------------------\n");
	dbg(" Command 14\t: icmpv4_echo <0/1> \n"
	...
	dbg("------------------------------------------------------------------------------\n");
	dbg(" Command 15\t: plmn_mcc <mcc1> <mcc2> <mcc3> ... \n"
	...
	dbg("------------------------------------------------------------------------------\n");
	dbg(" Command 19\t: gas_cb_delay <intval> \n"
	...
	dbg("------------------------------------------------------------------------------\n");
	dbg(" Command 42\t: di <STA_MAC> <Reason Code> <Reauth Delay> <URL> \n"
...
static void setTestParameters(char *testName, hspotApT *hspotap)
{
	...
	if (!strcmp(testName, "test5.2-AP1:NAI")) {
		dbg("3GPP enabled, NAI realm disabled\n");
		...
		dbg("NAI realm enabled, 3GPP disabled\n");
		...
		dbg("NAI realm disabled, 3GPP disabled\n");
		hspotap->isRealmDisabled = TRUE;
		hspotap->is3GppDisabled = TRUE;
...
int main(int argc, char **argv)
{
	...
	dbg("\n");
	dbg("Hotspot2.0 - version %s\n", EPI_VERSION_STR);
	dbg("Copyright Broadcom Corporation\n");

	if (wl() == NULL) {
		dbg("can't find wl interface\n");
		exit(1);
	}
	...
	while ((ifr = wlif(total_ifr)) != NULL) {
		...
		dbg("HS_TRACE : 1 main ; hspotap_num = %d ####	\n", hspotap_num);
		dbg("HS_TRACE : 2 main ; Prefix = %s ####	\n", prefix);
		...
		if (ptr) {
			if (!strToEther(ptr, &hspotap->iw_HESSID)) {
				dbg("wrong format hessid in NVRAM\n");
			}
		...
		while (eap) {
			...
			dbg("Auth Count for this EAP = %d\n", iA);
			eap = eap_right;
			iE++;
		}
		/* Fill Eap Count */
		hspotap->realmlist.realm[iR].eapCount = iE;
		dbg("Eap Count for this REALM = %d\n\n", iE);
		...
		dbg("Realm Count for this Program = %d\n", iR);
		}
		}
		/* Realm List ------------------------------------------------ */
		...
		if (ptr) {
			...
			if (strlen(ptrv) > 0) {
				...
				while ((i < BCM_DECODE_ANQP_MAX_PLMN) &&
					...
					dbg("OperatorList %d: name = %s, lang = %s\n", i,
		...
		if (ptr) {
			...
			if (strlen(ptrv) > 0) {
				...
				while ((i < BCM_DECODE_HSPOT_ANQP_MAX_OSU_PROVIDER) &&
					...
					dbg("OSU Server URI %d: URI = %s, Length = %d\n", i,
		...
		if (ptr) {
			...
			if (strlen(ptrv) > 0) {
				...
				while ((i < BCM_DECODE_HSPOT_ANQP_MAX_OSU_PROVIDER) &&
					...
					dbg("OSU Method %d: Method = %s, Length = %d\n", i,
						hspotap->osuplist.osuProvider[i].method,
						hspotap->osuplist.osuProvider[i].methodLength);
					i++;
				}
			}
		...
		if (ptr) {
			...
			if (strlen(ptrv) > 0) {
				...
				dbg("Provider # %d: OSU Friendly Name %d:"
					" name = %s, lang = %s\n", iP, iD,
		...
		if (ptr) {
			...
			if (strlen(ptrv) > 0) {
				...
				dbg("Provider # %d: OSU Serv Desc %d:"
					" name = %s, lang = %s\n", iP, iD,
		...
		strncpy_n(hspotap->anonai.nai, nvram_get(strcat_r(prefix, "anonai", varname)),
			BCM_DECODE_HSPOT_ANQP_MAX_NAI_SIZE + 1);
		hspotap->anonai.naiLen = strlen(hspotap->anonai.nai);

		if (hspotap->anonai.naiLen > 0) {
		...
		if (strlen(ptrv) > 0) {
			...
			while ((i < BCM_DECODE_ANQP_MAX_PLMN) &&
				...
				dbg("HomeRealmQueryList %d: name = %s, encoding = %d\n", i,
					hspotap->homeqlist.data[i].name,
		...
		strncpy_n(hspotap->prefix, prefix, MAX_NVPARSE);
			...
			dbg("can't find NVRAM ifname for %s\n", osifname);
		}
		...
		if (hspotap_num == 1) {
			for (i = 1; i < argc; i++) {
				if (strcmp(argv[i], "-help") == 0) {
					...
					dbg(" -verbose    		enable verbose output\n");
					dbg(" -help       		print this menu\n");
					dbg(" -dgaf       		disable DGAF\n");
					dbg(" -tcp_port <port>   Run hspotap in CLI mode\n");
					dbg("\n");
					dbg("To redirect to file use 'tee' "
						"(eg. %s -d | tee log.txt).\n", argv[0]);
					dbg("\n");
					hspotapFree();
					exit(1);
					...
					if (i == (argc - 1)) {
						dbg("Not enough args for tcp port option\n");
						hspotapFree();
						exit(1);
					}
					...
					if (i == (argc - 1)) {
						dbg("Not enough arguments for test option\n");
						hspotapFree();
						exit(1);
					}
					...
					if (i == (argc - 1)) {
						dbg("Not enough args for respSize option\n");
						hspotapFree();
						exit(1);
					}
