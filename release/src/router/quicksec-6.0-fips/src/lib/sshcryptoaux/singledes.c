...
Boolean ssh_single_des_is_weak_key(const unsigned char *key)
{
  int i;
  ...
  for (i = 0; i < SSH_DES_WEAK_KEYS; i++)
    {
      ...
      for (j = 0, match = 0; j < 8; j++)
        {
          if ((key[j] & 0xfe) != (ssh_single_des_weak_keys[i][j] & 0xfe))
            break;
...
SshUInt32 ssh_single_des_pc1_l(SshUInt32 i1, SshUInt32 i2)
{
  SshUInt32 t;
...
SshUInt32 ssh_single_des_pc1_r(SshUInt32 i1, SshUInt32 i2)
{
  SshUInt32 t;
...
SshUInt32 ssh_single_des_pc2_l(SshUInt32 x)
{
  SshUInt32 t;
...
SshUInt32 ssh_single_des_pc2_r(SshUInt32 x)
{
  SshUInt32 t;
...
static void
ssh_single_des_operate(SshUInt32 l, SshUInt32 r, SshUInt32 *output,
...
void ssh_single_des_set_key(const unsigned char *key, SshUInt32 *schedule)
{
  SshUInt32 c, d, tmp;
  SshUInt32 t1, t2;
  int i;
  ...
  tmp = ssh_single_des_pc1_l(c, d);
  d = ssh_single_des_pc1_r(c, d);
  c = tmp;
  ...
  for (i = 0; i < 16; i++)
    {
      ...
      t1 = ssh_single_des_pc2_l(c);
      t2 = ssh_single_des_pc2_r(d);
                           ...
                           size_t len)
{
  ...
  if (ssh_single_des_is_weak_key(key))
    {
      SSH_DEBUG(SSH_D_FAIL, ("Weak DES key, failing operation"));
      return FALSE;
    }

  ssh_single_des_set_key(key, key_schedule);

  /* IV not used, i.e. is zero at start */
  iv[0] = 0;
  iv[1] = 0;
