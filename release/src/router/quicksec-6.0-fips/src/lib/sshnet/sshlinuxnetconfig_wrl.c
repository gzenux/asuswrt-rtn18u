...
#ifdef HAVE_WRL_VRF
/* This define is needed in folowing included files */
...
/***************  Mapping between ifnums and routing instances  **************/
struct IfnumVrfMapStruct
{
  int ifnum;
...
struct IfnumVrfMapStruct ifnum_vrf_map[MAX_INTERFACES];

static void update_ifnum_vrf_map(int ifnum, int routing_instance_id)
{
  int i;

  /* update the ifnum if already there */
  for (i = 0; i < MAX_INTERFACES; i++)
    {
      if (ifnum_vrf_map[i].ifnum == 0) /* 0 is invalid */
        break;

      if (ifnum_vrf_map[i].ifnum == ifnum)
        {
          ifnum_vrf_map[i].routing_instance_id = routing_instance_id;
          return;
        }
    }
  ...
  if (i == MAX_INTERFACES)
    {
      SSH_DEBUG(SSH_D_ERROR, ("Max number of supported interfaces reached"));
      return;
    }

  /* Add the ifnum if not found */
  ifnum_vrf_map[i].ifnum = ifnum;
  ifnum_vrf_map[i].routing_instance_id = routing_instance_id;

}

static int get_routing_instance_by_ifnum(int ifnum)
{
  int i;
  ...
  for (i = 0; i < MAX_INTERFACES; i++)
    {
      if (ifnum_vrf_map[i].ifnum == ifnum)
        return ifnum_vrf_map[i].routing_instance_id;
    }

  SSH_DEBUG(SSH_D_LOWOK, ("Interface with ifnum %d not found.", ifnum));
  return 0;
}
                           ...
                           int routing_instance_id)
{
  ...
  /* Set socket routing instance. Windriver VRF kernel does not return info
   * about all interfaces for NLM_F_DUMP requests otherwise */
  SSH_DEBUG(SSH_D_LOWOK, ("Setting socket routing instance to %d",
                          routing_instance_id));
  if (setsockopt(sd, SOL_SOCKET, SO_VR_ID, (int *)&routing_instance_id,
             sizeof(routing_instance_id)) < 0)
    {
      SSH_DEBUG(SSH_D_FAIL,
               ("Setting socket routing instance failed, errno %d", errno));
    }
...
static Boolean
linux_netlink_get_link(SshLinuxNetlinkMsgParseCallback parse_cb,
                       void *context,
                       SshUInt32 ifnum)
{
  Boolean ret = FALSE;
  ...
  routing_instance_id = get_routing_instance_by_ifnum(ifnum);
...
static Boolean
linux_netlink_get_links_dump(SshLinuxNetlinkMsgParseCallback parse_cb,
                             void *context)
{
  Boolean ret = FALSE;
  SshLinuxNetlinkRequestStruct req;
  ...
  /* Send request, parse response and call callback for each response. */
  ret = linux_netlink_do_operation(&req, parse_cb, context, 0);
...
linux_netlink_set_link_ioctl(SshUInt32 ifnum, SshUInt32 mtu, SshUInt32 flags,
                             SshUInt32 mask, int routing_instance_id)
{
  int sd = 0;
  struct ifreq ifr;
  ...
  /* Set routing instance (VRF) */
#ifdef HAVE_WRL_VRF
  if (routing_instance_id >= 0)
    {
      ifr.ifr_vr_id = routing_instance_id;
      if (ioctl(sd, SIOCSIFVRID, &ifr))
        {
          SSH_DEBUG(SSH_D_ERROR, ("IOCTL failed errno %d", errno));
          goto fail;
        }
    }
#endif /* HAVE_WRL_VRF */
...
static Boolean
linux_netlink_get_addr(SshLinuxNetlinkMsgParseCallback parse_cb,
                       void *context,
                       int routing_instance_id)
{
  Boolean ret = FALSE;
  ...
  /* Send request, parse response and call callback for each response. */
  ret = linux_netlink_do_operation(&req, parse_cb,
                          ...
                          void *context)
{
  ...
  SSH_DEBUG(SSH_D_LOWSTART,
            ("%s address for ifnum %d routing instance %d",
            add ? "Add" : "Delete", ifnum, routing_instance_id));
...
ssh_netconfig_get_links(SshUInt32 *ifnums, SshUInt32 *num_ifnums)
{
  ...
  if (!linux_netlink_get_links_dump(netconfig_get_links_parser, &ctx))
    return SSH_NETCONFIG_ERROR_UNDEFINED;
...
ssh_netconfig_get_link(SshUInt32 ifnum, SshNetconfigLink link)
{
  ...
  if (!linux_netlink_get_link(netconfig_get_link_parser, &ctx, ifnum))
    return SSH_NETCONFIG_ERROR_UNDEFINED;
...
#ifdef HAVE_WRL_VRF
  if (linux_netlink_set_link_ioctl(ifnum, 0, 0, 0, routing_instance_id))
    {
      error = SSH_NETCONFIG_ERROR_OK;
      update_ifnum_vrf_map(ifnum, routing_instance_id);
    }
#endif /* HAVE_WRL_VRF */
...
ssh_netconfig_set_link_flags(SshUInt32 ifnum, SshUInt32 flags, SshUInt32 mask)
{
  ...
  if (linux_netlink_set_link_ioctl(ifnum, 0, flags, mask, -1));
    error = SSH_NETCONFIG_ERROR_OK;
#endif /* LINUX_HAS_RTM_SETLINK */
...
ssh_netconfig_set_link_mtu(SshUInt32 ifnum, SshUInt16 mtu)
{
  ...
  if (!linux_netlink_set_link(ifnum, mtu, 0, 0, link.routing_instance_id,
    ...
    return SSH_NETCONFIG_ERROR_UNDEFINED;
#else /*  LINUX_HAS_RTM_SETLINK*/
  if (linux_netlink_set_link_ioctl(ifnum, mtu, 0, 0, -1))
    error = SSH_NETCONFIG_ERROR_OK;
#endif /* LINUX_HAS_RTM_SETLINK */
...
ssh_netconfig_resolve_ifname(const unsigned char *ifname, SshUInt32 *ifnum_ret)
{
  ...
  if (!linux_netlink_get_links_dump(netconfig_resolve_ifname_parser, &ctx)
      || ctx.error != SSH_NETCONFIG_ERROR_OK)
    return ctx.error;
                            ...
                            size_t ifname_len)
{
  ...
  if (!linux_netlink_get_link(netconfig_resolve_ifname_parser, &ctx, ifnum)
      || ctx.error != SSH_NETCONFIG_ERROR_OK)
    return ctx.error;
