...
/** Allocates new linear buffer and copies all data from packet data
    buffer to it. In successful case returns pointer to linear buffer
    and length of packet in 'packet_len'. In failure case NULL is
    returned. */
unsigned char *
ssh_usermode_interceptor_packet_copy_to_buf(SshInterceptorPacket pp,
                                            size_t *packet_len);
#else /* SSH_USERMODE_INTERCEPTOR_ENABLE_TESTS */
/** Returns a pointer to the packet data buffer and length of packet
    in 'packet_len'. This is used when encoding packet data for sending
    to usermodeforwarder in kernel. This functions is only available
    when usermodeinterceptor packet API test features are disabled
    (as in this case the packet data buffer is always linearized). */
unsigned char *
