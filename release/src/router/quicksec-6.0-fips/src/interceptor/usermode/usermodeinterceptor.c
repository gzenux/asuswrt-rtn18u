 ...
 */
#define SSH_PACKET_DATA_BUF_LEN 65535

int kin_receive = 0;
int kwrite_len = 0;
int kwrite_cnt = 0;
unsigned char ksend_buf[SSH_PACKET_DATA_BUF_LEN];

char *data_context = "/proc/quicksec-usermode/data";
  ...
  {                                                                     \
      ...
      while                                                             \
        ...
        {                                                               \
          ...
          ssh_interceptor_packet_done_iteration_read(pp, &seg, &len);   \
        }                                                               \
...
#define SSH_PACKET_IDATA_HEADER_LEN_OFFSET     (8 * sizeof(SshUInt32))

/* Process a network packet message received from the kernel. Called
   in eloop context. */
size_t ssh_kernel_receive_data_packet(SshInterceptor interceptor,
                                      const unsigned char *data,
                                      size_t len)
{
  SshUInt32 packets, flags, ifnum_in, ifnum_out, protocol;
  ...
  packets = SSH_GET_32BIT(data);
  flags = SSH_GET_32BIT(data + SSH_PACKET_IDATA_FLAGS_OFFSET);
  ifnum_in = SSH_GET_32BIT(data + SSH_PACKET_IDATA_IFNUM_IN_OFFSET);
  ifnum_out = SSH_GET_32BIT(data + SSH_PACKET_IDATA_IFNUM_OUT_OFFSET);
  protocol = SSH_GET_32BIT(data + SSH_PACKET_IDATA_PROTO_OFFSET);
  internal_len = SSH_GET_32BIT(data + SSH_PACKET_IDATA_INTERNAL_LEN_OFFSET);
  internal_ptr = data + SSH_PACKET_IDATA_INTERNAL_DATA_OFFSET;
  packet_len =
    SSH_GET_32BIT(data + SSH_PACKET_IDATA_INTERNAL_DATA_OFFSET + internal_len);
  packet_ptr = data + SSH_PACKET_IDATA_HEADER_LEN_OFFSET  + internal_len;

  data_packet_len =
    SSH_PACKET_IDATA_HEADER_LEN_OFFSET + internal_len + packet_len;
  ...
  pp->routing_instance_id = (SshVriId) SSH_INTERCEPTOR_VRI_ID_GLOBAL;
  ...
  if (!ssh_interceptor_packet_copyin(pp, 0, packet_ptr, packet_len))
    {
      ...
      return SSH_PACKET_IDATA_HEADER_LEN_OFFSET + internal_len + packet_len;
    }
  ...
  for (i = 0; i < num_interfaces; i++)
    {
      ...
      ifs[i].routing_instance_id = (SshVriId)SSH_INTERCEPTOR_VRI_ID_GLOBAL;
      memcpy(ifs[i].routing_instance_name, SSH_INTERCEPTOR_VRI_NAME_GLOBAL,
             SSH_INTERCEPTOR_VRI_NAMESIZE);
      ifs[i].routing_instance_name[SSH_INTERCEPTOR_VRI_NAMESIZE - 1] = '\0';
...
void ssh_data_callback(unsigned int events, void *context)
{
  SshInterceptor interceptor = (SshInterceptor)context;
  size_t offset = 0;
  unsigned char rcvbuf[SSH_PACKET_DATA_BUF_LEN];
  ssize_t rcv_len;

  if (events & SSH_IO_CLOSED)
    {
      printf("Closed indication for data...\n");
    }
  ...
  if (events & SSH_IO_READ)
    {
      ...
      rcv_len = read(interceptor->data_file, (void *)rcvbuf,
                     SSH_PACKET_DATA_BUF_LEN);

      /* Read the buffer here. */
      while (offset < rcv_len)
        {
          offset += ssh_kernel_receive_data_packet(interceptor,
                                                   rcvbuf + offset,
                                                   rcv_len - offset);
        }
      ...
      if (kwrite_cnt != 0)
        {
          /* XXX Write the packet queue to the kernel. */
          SSH_PUT_32BIT(ksend_buf, kwrite_cnt);
          rcv_len = write(interceptor->data_file, ksend_buf, kwrite_len);
          SSH_ASSERT(rcv_len == kwrite_len);
          kwrite_cnt = kwrite_len = 0;
        }
    }

  if (events & SSH_IO_WRITE)
    {
      printf("Write indication for data...\n");
    }
}
...
Boolean ssh_interceptor_init(void *machine_context)
{
  ...
  interceptor->data_file = open((char *)data_context, O_RDWR);
  if (interceptor->data_file < 0)
    {
  ...
  if (ssh_io_register_fd(interceptor->data_file,
                         ssh_data_callback, interceptor) == FALSE)
    printf("Failed to register FD\n");
  else
    printf("Succeedeed to register FD %d\n", interceptor->data_file);

  ssh_io_set_fd_request(interceptor->data_file, SSH_IO_READ);
                          ...
                          size_t media_header_len)
{
  ...
  if (kin_receive == 0)
    SSH_PUT_32BIT(ksend_buf, 1);
  else /* kin_receive > 0 */
    {
      if ((packet_len + 1024 + kwrite_len) >= SSH_PACKET_DATA_BUF_LEN)
        {
          SSH_PUT_32BIT(ksend_buf, kwrite_cnt);

          write_l = write(interceptor->data_file, ksend_buf, kwrite_len);
          SSH_ASSERT(write_l == kwrite_len);
          kwrite_len = 0;
          kwrite_cnt = 0;
        }
    }

  SSH_PUT_32BIT(ksend_buf + SSH_PACKET_UDATA_FLAGS_OFFSET + kwrite_len,
                pp->flags);
  SSH_PUT_32BIT(ksend_buf + SSH_PACKET_UDATA_IFNUM_IN_OFFSET + kwrite_len,
                pp->ifnum_in);
  SSH_PUT_32BIT(ksend_buf + SSH_PACKET_UDATA_IFNUM_OUT_OFFSET + kwrite_len,
                pp->ifnum_out);
  SSH_PUT_32BIT(ksend_buf + SSH_PACKET_UDATA_PROTO_OFFSET + kwrite_len,
                pp->protocol);
  SSH_PUT_32BIT(ksend_buf + SSH_PACKET_UDATA_MEDIA_HDR_LEN_OFFSET + kwrite_len,
                media_header_len);
#ifdef SSH_IPSEC_IP_ONLY_INTERCEPTOR
  SSH_PUT_32BIT(ksend_buf + SSH_PACKET_UDATA_ROUTE_SELECTOR_OFFSET +
                kwrite_len,
                pp->route_selector);
#endif /* SSH_IPSEC_IP_ONLY_INTERCEPTOR */
  SSH_PUT_32BIT(ksend_buf + SSH_PACKET_UDATA_PACKET_LEN_OFFSET + kwrite_len,
                packet_len);
  SSH_PUT_32BIT(ksend_buf + SSH_PACKET_UDATA_INTERNAL_LEN_OFFSET + kwrite_len,
                internal_len);
  memcpy(ksend_buf + SSH_PACKET_UDATA_INTERNAL_DATA_OFFSET + kwrite_len,
  ...
  if (kin_receive)
    {
      kwrite_len +=
        SSH_PACKET_UDATA_INTERNAL_DATA_OFFSET + packet_len + internal_len;
      kwrite_cnt++;
  ...
  /* Not a good way to do, but to prevent duplicate internal data
     discarded messages, we need to drop the internal data here. */
#ifdef INTERCEPTOR_HAS_PACKET_INTERNAL_DATA_ROUTINES
  ssh_interceptor_packet_free_internal_data(pp);
#endif /* INTERCEPTOR_HAS_PACKET_INTERNAL_DATA_ROUTINES */
...
void ssh_interceptor_uninit(void)
{
 ...
 ssh_io_unregister_fd(interceptor->data_file, TRUE);
 close(interceptor->data_file);
                           ...
                           void *context)
{
  ...
  if (op == NULL)
    ...
    lookup_failure:
      SSH_USERMODE_INTERCEPTOR_UNLOCK(interceptor);
      (*completion)(FALSE, NULL, 0, -1, 0, context);
      return;
    }
