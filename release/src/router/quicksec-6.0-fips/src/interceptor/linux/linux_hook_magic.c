...
static unsigned int
ssh_interceptor_hook_magic_hookfn_ipv4(SshNfHooknum hooknum,
                                       SshHookSkb *skb,
                                       const struct net_device *in,
                                       const struct net_device *out,
                                       int (*okfn) (struct sk_buff *))
{
  return ssh_interceptor_hook_magic_hookfn(PF_INET, SSH_GET_HOOKNUM(hooknum),
                                           skb, in, out, okfn);
}

#ifdef SSH_LINUX_INTERCEPTOR_IPV6
static unsigned int
ssh_interceptor_hook_magic_hookfn_ipv6(SshNfHooknum hooknum,
                                       SshHookSkb *skb,
                                       const struct net_device *in,
                                       const struct net_device *out,
                                       int (*okfn) (struct sk_buff *))
{
  return ssh_interceptor_hook_magic_hookfn(PF_INET6, SSH_GET_HOOKNUM(hooknum),
                                           skb, in, out, okfn);
}
#endif /* SSH_LINUX_INTERCEPTOR_IPV6 */
...
#ifndef SSH_IPSEC_IP_ONLY_INTERCEPTOR
static unsigned int
ssh_interceptor_hook_magic_hookfn_arp(SshNfHooknum hooknum,
                                      SshHookSkb *skb,
                                      const struct net_device *in,
                                      const struct net_device *out,
                                      int (*okfn) (struct sk_buff *))
{
  return ssh_interceptor_hook_magic_hookfn(SSH_NFPROTO_ARP,
                                           SSH_GET_HOOKNUM(hooknum),
                                           skb, in, out, okfn);
}
#endif /* !SSH_IPSEC_IP_ONLY_INTERCEPTOR */
  ...
  SSH_INTERCEPTOR_VRF_FOR_EACH_VRFID(routing_instance_id)
    {
      ...
      for (dev = SSH_FIRST_NET_DEVICE(net);
           ...
           dev = SSH_NEXT_NET_DEVICE(dev))
        {
          ...
          /* This device is suitable for probing. We only need one device. */
          goto found_dev;
    ...
    } /* SSH_INTERCEPTOR_VRF_FOR_EACH_VRFID */
