...
interceptor_route_create_child_dst(SshInterceptor interceptor,
                                   ...
                                   Boolean ipv6)
{
  ...
  ssh_kernel_mutex_lock(interceptor->nf->route_lock);

  /* Now that we have the lock, check that dst_entry does not have
     a child entry */
  if (dst->child != NULL)
    {
      ssh_kernel_mutex_unlock(interceptor->nf->route_lock);
      return dst->child;
    }
                                               ...
                                               SshVriId routing_instance_id)
{
  ...
  if (ssh_interceptor_is_valid_vrf_by_id(routing_instance_id)
      == FALSE)
    goto failed;
...
#ifdef HAVE_WRL_VRF
    fli->vr_id = routing_instance_id;
#endif /* HAVE_WRL_VRF */

  rval = afinfo->route(net, &dst, fli, TRUE);
  rcu_read_unlock();
  ...
  *rt = (struct rtable *)dst;
...

  {
    ...
    rval = ip_route_output_key(net, rt, fli);
  }
                                                  ...
                                                  SshVriId routing_instance_id)
{
  ...
  rval = afinfo->route(net, &dst, fli, FALSE);
  rcu_read_unlock();
  if (rval != 0)
    {
...

  {
    ...
    dst = ip6_route_output(net, NULL, fli);
  }
#endif /* LINUX_USE_NF_FOR_ROUTE_OUTPUT */
                                  ...
                                  SshInterceptorRouteResult result)
{
  ...
  if (selector & SSH_INTERCEPTOR_ROUTE_KEY_RIID)
    routing_instance_id = key->routing_instance_id;
  else
    routing_instance_id = SSH_INTERCEPTOR_VRI_ID_GLOBAL;
  ...
  SSH_DEBUG(SSH_D_LOWOK,
            ("Route lookup: "
             ...
             ((selector & SSH_INTERCEPTOR_ROUTE_KEY_EXTENSION) ?
              fwmark : 0), routing_instance_id));

  /* Perform route lookup */
  rval = interceptor_ip4_route_output(&rt, &rt_key, routing_instance_id);
  if (rval < 0)
    goto fail;
  ...
  if (!rt->rt_gateway)
    {
      *result->gw = key->dst;
      SSH_DEBUG(SSH_D_NICETOKNOW,
          ("RT did not use gateway, copying destination %@",
           ssh_ipaddr_render, &key->dst));
    }
  ...
  SSH_DEBUG(SSH_D_LOWOK,
            ("Route result: dst %@ via %@ ifnum %d[%s] mtu %d type %s [%d]"
             ...
             result->mtu, rt_type_str, rt_type, routing_instance_id));
  ...
  if ((selector & SSH_INTERCEPTOR_ROUTE_KEY_FLAG_TRANSFORM_APPLIED)
    ...
    {
      if (interceptor_route_create_child_dst(interceptor, &SSH_RT_DST(rt),
                                             FALSE) == NULL)
                                 ...
                                 SshInterceptorRouteResult result)
{
  ...
  SSH_ASSERT((selector & SSH_INTERCEPTOR_ROUTE_KEY_SRC) != 0);
  SSH_IP4_ENCODE(&key->src, (unsigned char *) &saddr);

  SSH_ASSERT((selector & SSH_INTERCEPTOR_ROUTE_KEY_IN_IFNUM) != 0);
  SSH_LINUX_ASSERT_VALID_IFNUM(key->ifnum);
  ...
  SSH_DEBUG(SSH_D_NICETOKNOW, ("Routing instance id is %s set, using "
                               "device as selector for routing.",
                               (selector & SSH_INTERCEPTOR_ROUTE_KEY_RIID)
                               != 0 ? "" : "not"));
  if (skbp->dev == NULL)
    {
      SSH_DEBUG(SSH_D_NICETOKNOW,
                ("Set temporarily dev to skb (and routing instance)."));
      ssh_skb_set_vrf_by_id(skbp, key->routing_instance_id);
      skbp->dev = dev;
    }
  ...
  SSH_DEBUG(SSH_D_LOWOK,
            ("Route result: dst %@ via %@ ifnum %d[%s] mtu %d type %s [%d]"
             " routing instance id %d (in key %d)",
             ssh_ipaddr_render, &key->dst,
             ssh_ipaddr_render, result->gw, result->ifnum,
             (SSH_RT_DST(rt).dev->name ? SSH_RT_DST(rt).dev->name : "none"),
             result->mtu, rt_type_str, rt_type,
             ssh_interceptor_get_dev_vrf_id(skb_dst(skbp)->dev),
             key->routing_instance_id));

  if ((selector & SSH_INTERCEPTOR_ROUTE_KEY_RIID) != 0 &&
      ssh_interceptor_get_dev_vrf_id(skb_dst(skbp)->dev) !=
      key->routing_instance_id)
    {
      skb_dst_drop(skbp);
  ...
  if ((selector & SSH_INTERCEPTOR_ROUTE_KEY_FLAG_TRANSFORM_APPLIED)
    ...
    {
      if (interceptor_route_create_child_dst(interceptor, skb_dst(skbp), FALSE)
          == NULL)
        SSH_DEBUG(SSH_D_FAIL, ("Could not create child dst_entry for dst %p",
                               skb_dst(skbp)));
    }
                                  ...
                                  SshInterceptorRouteResult result)
{
  ...
  SSH_DEBUG(SSH_D_LOWOK,
            ("Route lookup: "
             "dst %@ src %@ ifnum %d routing instance id %d",
             ssh_ipaddr_render, &key->dst,
             ssh_ipaddr_render,
             ((selector & SSH_INTERCEPTOR_ROUTE_KEY_SRC) ?
              &key->src : NULL),
             ((selector & SSH_INTERCEPTOR_ROUTE_KEY_OUT_IFNUM) ?
              key->ifnum : -1), routing_instance_id));

  dst = interceptor_ip6_route_output(&rt_key, routing_instance_id);
  if (dst == NULL)
    {
      goto fail;
    }
  ...
  if (neigh != NULL)
  ...
  else
    {
      ...
      /* Ok, RT does not have gateway, let's try to send this directly to
         destination. */
      SSH_DEBUG(SSH_D_NICETOKNOW,
                ("RT6 did not use gateway, copying destination",
                 ssh_ipaddr_render, &key->dst));
  ...
  result->mtu = SSH_LINUX_DST_MTU(dst);
  ...
  if ((selector & SSH_INTERCEPTOR_ROUTE_KEY_FLAG_TRANSFORM_APPLIED)
      && dst->child == NULL)
    {
      if (interceptor_route_create_child_dst(interceptor, dst, TRUE) == NULL)
          SSH_DEBUG(SSH_D_FAIL, ("Could not create child dst_entry for dst %p",
                                 dst));
    }
  ...
  if (SSH_IP_IS4(&key->dst))
    {
      ...
      (*callback)(TRUE, result.gw, result.ifnum, key->routing_instance_id,
                                 ...
                                 SshVriId routing_instance_id)
{
  ...
  if ((route_selector & SSH_INTERCEPTOR_ROUTE_KEY_SRC)
      ...
      && (route_selector & SSH_INTERCEPTOR_ROUTE_KEY_IN_IFNUM))
    {
      ...
      /* Set the routing information for device */
      if (route_selector & SSH_INTERCEPTOR_ROUTE_KEY_RIID)
        {
          if (skbp->dev == NULL)
            {
              SSH_DEBUG(SSH_D_NICETOKNOW,
                        ("Set temporarily dev for skbp."));
              skbp->dev = dev;
              dev_taken = 1;
            }
        }
  ...
  else
    {
      ...
      if (route_selector & SSH_INTERCEPTOR_ROUTE_KEY_RIID)
        riid = routing_instance_id;
      else
        riid = SSH_INTERCEPTOR_VRI_ID_GLOBAL;
      ...
      rval = interceptor_ip4_route_output(&rt, &rt_key, riid);
      if (rval < 0)
        {
          SSH_DEBUG(SSH_D_FAIL,
  ...
  if (route_selector & SSH_INTERCEPTOR_ROUTE_KEY_FLAG_TRANSFORM_APPLIED)
    {
      ...
      if (dst->child == NULL)
        {
          if (interceptor_route_create_child_dst(interceptor, dst, FALSE)
              == NULL)
            {
                                 ...
                                 SshVriId routing_instance_id)
{
  ...
  dst = interceptor_ip6_route_output(&rt_key, riid);
  ...
  if (route_selector & SSH_INTERCEPTOR_ROUTE_KEY_FLAG_TRANSFORM_APPLIED)
    {
      ...
      if (dst->child == NULL)
        {
          if (interceptor_route_create_child_dst(interceptor, dst, TRUE)
              == NULL)
            {
              SSH_DEBUG(SSH_D_ERROR,
                        ("Could not create child dst_entry for dst %p",
