...
module_param(ssh_debug_level, uint, 0664);
#endif /* DEBUG_LIGHT */
...
ssh_interceptor_notify_ipm_open(SshInterceptor interceptor)
{
  ...
  interceptor->enable_interception = TRUE;
...
ssh_interceptor_uninit_kernel_services(void)
{
  ...
  /* Uninitialize data channel */
  ssh_interceptor_ud_uninit(ssh_interceptor_context);
...
ssh_interceptor_init_kernel_services(void)
{
  ...
  if (ssh_interceptor_dst_entry_cache_init(ssh_interceptor_context) == FALSE)
    {
      printk(KERN_ERR "QuickSec packet processing engine "
  ...
  if (!ssh_interceptor_ipm_init(ssh_interceptor_context))
    {
      printk(KERN_ERR
             "QuickSec packet processing engine failed to start "
             "(proc filesystem initialization error)\n");
      goto error;
    }

  if (!ssh_interceptor_ud_init(ssh_interceptor_context))
    {
      printk(KERN_ERR
             "QuickSec packet processing engine failed to start "
             "(proc filesystem initialization error)\n");
...
int ssh_interceptor_init(void)
{
  ...
  printk(KERN_INFO "QuickSec built on " __DATE__ " " __TIME__ "\n");
