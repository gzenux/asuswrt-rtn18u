...
#define SSH_LINUX_INTERCEPTOR_MODULE_DESCRIPTION "QuickSec"
...
#endif /* defined(LINUX_SSH_RTABLE_FIRST_MEMBER_RT_GENID) */
...
typedef struct SshInterceptorUserdataRec
{
  /* RW lock for protecting the send message queue and the message freelist. */
  rwlock_t lock;
  ...
  /* Output message queue */
  SshInterceptorPacket send_queue;
  SshInterceptorPacket send_queue_tail;

  SshUInt32 in_queue_cnt;
  SshUInt32 in_queue_bytes;

} SshInterceptorUserdataStruct, *SshInterceptorUserdata;
...
/* Structure for other /proc entries. */
typedef struct SshInterceptorUserdataEntryRec
{
  /* /proc filesystem inode */
  struct proc_dir_entry *entry;
...
} SshInterceptorUserdataEntryStruct, *SshInterceptorUserdataEntry;
...
struct SshInterceptorRec
{
  ...
  /* Userdata channel */
  SshInterceptorUserdataStruct ud;
  ...
  SshInterceptorUserdataEntryStruct ud_proc_entry;
...
size_t
ssh_engine_from_ipm_data_packet(SshEngine engine,
                                unsigned char *rcvbuf,
                                size_t len);

size_t
ssh_engine_from_ipm_user_data_packet(SshEngine engine,
				     const char __user *rcvbuf,
				     size_t len);
