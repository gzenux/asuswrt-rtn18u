...
int
bcm6300_a0_arl_write(int unit, int port, int vlan, const bmd_mac_addr_t *mac_addr)
{
    int ioerr = 0;
    int rv;
    ARLA_MACr_t arla_mac;
    ARLA_VIDr_t arla_vid;
    ARLA_MACVID_ENTRY0r_t arla_macvid;
    ARLA_FWD_ENTRY0r_t arla_fwd;
...
int
bcm6300_a0_bmd_port_mac_addr_add(int unit, int port, int vlan, const bmd_mac_addr_t *mac_addr)
{
    BMD_CHECK_UNIT(unit);
    BMD_CHECK_VLAN(unit, vlan);
    BMD_CHECK_PORT(unit, port);

    return bcm6300_a0_arl_write(unit, port, vlan, mac_addr);
}
