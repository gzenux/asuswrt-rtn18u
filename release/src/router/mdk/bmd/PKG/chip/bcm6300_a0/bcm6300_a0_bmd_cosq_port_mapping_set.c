...
int 
bcm6300_a0_bmd_cosq_port_mapping_set(
    int unit, 
    int port, 
    int priority, 
    int egressq)
{
    ...
    if (NUM_EGRESS_QUEUES == 8)
        queue_bits = 3;
    else 
        queue_bits = 2;

    ioerr += READ_QOS_RX_CTRL_Pr(unit, port, &ctrl);
    ctrl.qos_rx_ctrl_p[0] &= ~(0x3 << (priority * queue_bits) );
    ctrl.qos_rx_ctrl_p[0] |= (egressq << (priority * queue_bits) );
    ioerr += WRITE_QOS_RX_CTRL_Pr(unit, port, ctrl);
