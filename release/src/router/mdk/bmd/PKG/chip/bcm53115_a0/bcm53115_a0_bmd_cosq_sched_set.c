...
/*
 * $Id: bcm53115_a0_bmd_cosq_sched_set.c,v 1.2 Broadcom SDK $
 * 
...
int 
bcm53115_a0_bmd_cosq_sched_set(
    int unit, 
    bmd_cosq_sched_t sched_type, 
    int sp_endq)
{
    int ioerr = 0, txq_mode = 0;
    QOS_TX_CTRLr_t ctrl;
    ...
    if (sched_type == bmdWeightedRoundRobin) {
        txq_mode = 0;
    } else if (sched_type == bmdStrictPriority) {
        txq_mode = 3;
    } else if (sched_type == bmdSpWrrCombo) {
        txq_mode = sp_endq; 
    } else {
        CDK_ERR(("Invalid scheduling mode requested!!!."));
        return CDK_E_PARAM;
    }
    QOS_TX_CTRLr_QOS_PRIORITY_CTRLf_SET(ctrl, txq_mode);
