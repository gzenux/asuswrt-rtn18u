...
/*
 * $Id: bcm53115_a0_bmd_switch_control_priority_set.c,v 1.2 Broadcom SDK $
 * 
...
int 
bcm53115_a0_bmd_switch_control_priority_set(
    int unit, 
    int priority, 
    ...
    int value)
{
    ...
    switch (type) {
        case bmdSwitchTxQHiHysteresisThreshold:
            ...
            FC_TXQ_TH_RSRV_Qr_TXQ_HYST_THRSf_SET(txq_hyst, value);
            ioerr += WRITE_FC_TXQ_TH_RSRV_Qr(unit, priority, txq_hyst);
            break;

        case bmdSwitchTxQHiPauseThreshold:
            ioerr += READ_FC_TXQ_TH_PAUSE_Qr(unit, priority, &txq_pause);
            FC_TXQ_TH_PAUSE_Qr_TXQ_PAUSE_THRSf_SET(txq_pause, value);
            ioerr += WRITE_FC_TXQ_TH_PAUSE_Qr(unit, priority, txq_pause);
            break;

        case bmdSwitchTxQHiDropThreshold:
            ioerr += READ_FC_TXQ_TH_DROP_Qr(unit, priority, &txq_drop);
            FC_TXQ_TH_DROP_Qr_TXQ_DROP_THRSf_SET(txq_drop, value);
            ioerr += WRITE_FC_TXQ_TH_DROP_Qr(unit, priority, txq_drop);
            break;

        case bmdSwitchTotalHysteresisThreshold:
            ioerr += READ_FC_TOTAL_TH_HYST_Qr(unit, priority, &total_hyst);
            FC_TOTAL_TH_HYST_Qr_TXQ_TOTAL_HYST_THRSf_SET(total_hyst, value);
            ioerr += WRITE_FC_TOTAL_TH_HYST_Qr(unit, priority, total_hyst);
            break;

        case bmdSwitchTotalPauseThreshold:
            ioerr += READ_FC_TOTAL_TH_PAUSE_Qr(unit, priority, &total_pause);
            FC_TOTAL_TH_PAUSE_Qr_TXQ_TOTAL_PAUSE_THRSf_SET(total_pause, value);
            ioerr += WRITE_FC_TOTAL_TH_PAUSE_Qr(unit, priority, total_pause);
            break;

        case bmdSwitchTotalDropThreshold:
            ioerr += READ_FC_TOTAL_TH_DROP_Qr(unit, priority, &total_drop);
            FC_TOTAL_TH_DROP_Qr_TXQ_TOTAL_DROP_THRSf_SET(total_drop, value);
            ioerr += WRITE_FC_TOTAL_TH_DROP_Qr(unit, priority, total_drop);
            break;
