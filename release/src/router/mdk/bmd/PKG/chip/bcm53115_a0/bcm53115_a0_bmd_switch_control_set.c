...
/*
 * $Id: bcm53115_a0_bmd_switch_control_set.c,v 1.2 Broadcom SDK $
 * 
...
int 
bcm53115_a0_bmd_switch_control_set(
    int unit, 
    ...
    int value)
{
    ...
    switch (type) {
        case bmdSwitchTXQPauseControl:
            ...
            FC_PAUSE_DROP_CTRLr_EN_TX_PAUSEf_SET(fc_ctrl, value);
            ioerr += WRITE_FC_PAUSE_DROP_CTRLr(unit, fc_ctrl);
            break;

        case bmdSwitchTXQDropControl:
            ioerr += READ_FC_PAUSE_DROP_CTRLr(unit, &fc_ctrl);
            FC_PAUSE_DROP_CTRLr_EN_TX_DROPf_SET(fc_ctrl, value);
            ioerr += WRITE_FC_PAUSE_DROP_CTRLr(unit, fc_ctrl);
            break;

        case bmdSwitchTotalPauseControl:
            ioerr += READ_FC_PAUSE_DROP_CTRLr(unit, &fc_ctrl);
            FC_PAUSE_DROP_CTRLr_EN_TOTAL_PAUSEf_SET(fc_ctrl, value);
            ioerr += WRITE_FC_PAUSE_DROP_CTRLr(unit, fc_ctrl);
            break;

        case bmdSwitchTotalDropControl:
            ioerr += READ_FC_PAUSE_DROP_CTRLr(unit, &fc_ctrl);
            FC_PAUSE_DROP_CTRLr_EN_TOTAL_DROPf_SET(fc_ctrl, value);
            ioerr += WRITE_FC_PAUSE_DROP_CTRLr(unit, fc_ctrl);
            break;

        case bmdSwitch8021QControl:
            ioerr += READ_VLAN_CTRL0r(unit, &vlan_ctrl);
            VLAN_CTRL0r_VLAN_ENf_SET(vlan_ctrl, value);
            ioerr += WRITE_VLAN_CTRL0r(unit, vlan_ctrl);
            break;
