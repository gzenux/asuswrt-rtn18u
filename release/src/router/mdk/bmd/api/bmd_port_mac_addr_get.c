/*
 * $Id: bmd_port_mac_addr_add.c,v 1.7 Broadcom SDK $
 * 
...
extern int _bd##_bmd_port_mac_addr_get(int unit, int port, int num_req_entries, bmd_arl_entry_t *arl_entries, int *num_entries);
#include <bmdi/bmd_devlist.h>

/* Dispatch table for this function, which uses the externs above */
#define BMD_DEVLIST_ENTRY(_nm,_vn,_dv,_rv,_md,_pi,_bd,_bc,_fn,_fl,_cn,_pf,_pd,_r0,_r1) \
_bd##_bmd_port_mac_addr_get,
static int (*__dispatch_table[])(int unit, int port, int num_req_entries, bmd_arl_entry_t *arl_entries, int *num_entries) =
{
#include <bmdi/bmd_devlist.h>
0
};
...
int 
bmd_port_mac_addr_get(
    int unit, 
    int port,
    int num_req_entries,
    bmd_arl_entry_t *arl_entries,
    int *num_entries)
{
    ...
    return __dispatch_table[dev_type](unit, port, num_req_entries, arl_entries, num_entries);
}
