...
extern int _bd##_bmd_mcast_mac_addr_set(int unit, int entry_id, int vlan, bmd_mac_addr_t *mac_addr, int fwd_portmap, int priority);
#include <bmdi/bmd_devlist.h>

/* Dispatch table for this function, which uses the externs above */
#define BMD_DEVLIST_ENTRY(_nm,_vn,_dv,_rv,_md,_pi,_bd,_bc,_fn,_fl,_cn,_pf,_pd,_r0,_r1) \
_bd##_bmd_mcast_mac_addr_set,
static int (*__dispatch_table[])(int unit, int entry_id, int vlan, bmd_mac_addr_t *mac_addr, int fwd_portmap, int priority) =
{
#include <bmdi/bmd_devlist.h>
0
};
    ...
    int valid)
{
    ...
    return __dispatch_table[dev_type](unit, entry_id, vlan, mac_addr, fwd_portmap, priority);
}
