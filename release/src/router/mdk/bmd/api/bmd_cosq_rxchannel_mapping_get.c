...
extern int _bd##_bmd_cosq_rxchannel_mapping_get(int unit, int egressq, int *channel);
#include <bmdi/bmd_devlist.h>

/* Dispatch table for this function, which uses the externs above */
#define BMD_DEVLIST_ENTRY(_nm,_vn,_dv,_rv,_md,_pi,_bd,_bc,_fn,_fl,_cn,_pf,_pd,_r0,_r1) \
_bd##_bmd_cosq_rxchannel_mapping_get,
static int (*__dispatch_table[])(int unit, int egressq, int *channel) =
{
#include <bmdi/bmd_devlist.h>
0
};
...
int 
bmd_cosq_rxchannel_mapping_get(
    int unit, 
    int egressq, 
    int *channel)
{
    ...
    return __dispatch_table[dev_type](unit, egressq, channel);
}
