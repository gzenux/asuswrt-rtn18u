...
extern int _bd##_bmd_port_rate_ingress_get(int unit, int port, bmd_pkt_type_mask_t *pkt_type, uint32_t *kbits_sec, uint32_t *kbits_burst);
#include <bmdi/bmd_devlist.h>

/* Dispatch table for this function, which uses the externs above */
#define BMD_DEVLIST_ENTRY(_nm,_vn,_dv,_rv,_md,_pi,_bd,_bc,_fn,_fl,_cn,_pf,_pd,_r0,_r1) \
_bd##_bmd_port_rate_ingress_get,
static int (*__dispatch_table[])(int unit, int port, bmd_pkt_type_mask_t *pkt_type, uint32_t *kbits_sec, uint32_t *kbits_burst) =
{
#include <bmdi/bmd_devlist.h>
0
};
    ...
    uint32_t *kbits_burst)
{
    ...
    return __dispatch_table[dev_type](unit, port, pkt_type, kbits_sec, kbits_burst);
}
