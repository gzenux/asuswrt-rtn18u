...
#include <errno.h>
#include "ms_url_parser.h"
#include "ms_http_parser.h"
                        ...
                        if( strcmp(header + (total_read - 4), "\r\n\r\n")== 0 )
                            {
                                ...
                                //fprintf(stderr,"~~~~~~~~content_type is %s ~~~~~~~~~~\n",content_type);
...
{
       //fprintf(stderr, "enter http_transfer() \n");
        FILE *out 		= NULL;
        Url *u			= NULL;
        ...
        */
        ...
        if( ! (buf[0] == 'H' && buf[1] == 'T'
               ...
               return  P_FAIL;
                ...
                //fprintf(stderr, "get_raw_header:%s\n",raw_header);

                raw_header = strconcat(buf, raw_header, NULL);
                header = make_http_header(raw_header);

                //fprintf(stderr, "raw_header:%s\n",raw_header);

                //fprintf(stderr, "raw_header[9]:%d\n",raw_header[9]);
                ...
                if (raw_header[9] == '3' && new_location ) {
                        ...
                        //fprintf(stderr, "##########new_location value is #########\n");
                        //fprintf(stderr, "full url is %s \n",redir_u->full_url);
                        //fprintf(stderr, "host is %s \n",redir_u->host);
                        //fprintf(stderr, "port is %d \n",redir_u->port);
                        //fprintf(stderr, "service type is %d \n",redir_u->service_type);
