...
#endif
#if defined(HND_ROUTER) && defined(RTCONFIG_BWDPI)
	setup_dpi_conf_bit(2); // close IPS to avoid firmware upgrade blocked
#endif
...
#endif
#if defined(HND_ROUTER) && defined(RTCONFIG_BWDPI)
			/* recover whole setting in bw_dpi_conf */
			setup_dpi_conf_bit(-1);
#endif
		}
	}
}
