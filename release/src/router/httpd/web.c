			...
			while (tmp_result) {
				...
				for(r = 0; r < tmp_result->row_count; ++r){
					...
					for(c = 0; c < tmp_result->col_count; ++c, ++i){
						if(c != 0)
							websWrite(wp, ", ");
						websWrite(wp, "%s", tmp_result->result[i]);
					}
				}
