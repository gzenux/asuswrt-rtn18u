 ...
 *
 * $Id: acs.c 479523 2014-05-21 12:24:22Z $
 */
...
acs_dump_config_extra(acs_chaninfo_t *c_info)
{
	acs_fcs_t *fcs_info = &c_info->acs_fcs;
        ...
        ACSD_FCS("\t acs_dfs: %d\n", fcs_info->acs_dfs);
        ACSD_FCS("\t acs_lowband_least_rssi: %d\n", fcs_info->acs_lowband_least_rssi);
...
acs_pick_chanspec_common(acs_chaninfo_t *c_info, int bw, int score_type)
{
	...
	for (i = 0; i < c_info->c_count[bw]; i++) {
		...
		if (AUTOCHANNEL(c_info) && CHSPEC_IS2G(candi[i].chspec)
...
recheck:
		...
		for (i = 0; i < c_info->c_count[bw]; i++) {
			...
			if (!acs_is_initial_selection(c_info)) {
				...
				if (candi[i].is_dfs && (dfsr_disable ||
                                        (!acs_dfs_channel_is_usable(c_info,
                                        candi[i].chspec)))) {
					candi[i].reason |= ACS_INVALID_DFS;
				}
			}
	...
	if (c_info->dfs_forced_chspec == 0) {
		c_info->dfs_forced_chspec = selected;
#if 0
		ACSD_PRINT("selected DFS-exit channel spec: 0x%4x\n", selected);
#else
		ACSD_PRINT("selected DFS-exit channel spec: 0x%4x (%s)\n", selected,
			wf_chspec_ntoa(selected, chanspecbuf));
#endif
	}
	else {
...
acs_set_dfs_forced_chspec(acs_chaninfo_t * c_info)
{
        ...
        ret = wl_iovar_getint(c_info->name, "dfs_channel_forced", (void *)&val);
        if (ret < 0) {
                ACSD_ERROR("get dfs forced chanspec fails!\n");
                return;
        }

        ACSD_INFO("get dfs forced chanspec: 0x%x!\n", val);
        if (val == 0) {
                ret = wl_iovar_setint(c_info->name, "dfs_channel_forced", htod32(chspec));
...
acs_scan_timer_or_dfsr_check(acs_chaninfo_t * c_info)
{
	...
	if (AUTOCHANNEL(c_info) && !(c_info->cur_is_dfs)) {
...
int acs_update_assoc_info(acs_chaninfo_t *c_info)
{
        ...
        for (cnt = 0; cnt < list->count; cnt++) {
                ...
                if (acs_assoclist->sta_info[cnt].rssi < fcs_info->acs_lowband_least_rssi)
                        c_info->sta_status |= ACS_STA_EXIST_FAR;
                else
